...
//
// $URL: https://github.com/CGAL/cgal/blob/v5.5.1/BGL/include/CGAL/draw_face_graph.h $
...
protected:
  ...
  {
    using Point =
        typename boost::property_map_value<Graph, CGAL::vertex_point_t>::type;
    ...
    return [this, &g, vnormals, anofaces, fcolor, point_pmap]()
    {
      ...
      if (!anofaces)
      {
        for (auto fh: faces(g))
        {
          if (fh!=boost::graph_traits<Graph>::null_face())
          {
            CGAL::IO::Color c=fcolor(g, fh);
            face_begin(c);
